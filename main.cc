...
//    exit(0);
  ...
  //      exit(1);
      ...
      //	exit(1);
          ...
          else if(command[0].compare("-cellparams") == 0){ // ML mod: output cell parameters a b c alpha beta gamma
            string filename = processFilename(command, name, ".pbc", 0, 1);
            if(filename.empty()) {error=true; break;}
            cout << filename << endl;
            writeCellParams(&atmnet, (char *)filename.data());
          }
